...
public partial class ListarFacturas : System.Web.UI.Page
{
    ...
    void Listar()
    {
        try
        {
            ...
            string comprobante = txtFactura.Text.Trim();

            SqlDataAdapter da = new SqlDataAdapter("Play_Comprobante_Listar '" + FechaInicial + "','" + FechaFinal + "'," + Estado + ",'" + Almacen + "',2,'" + comprobante + "'", conexion);
    ...
    protected void gvPedido_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (lbIdPedido != null)
            {
                lbIdPedido.PostBackUrl = "CrearPedido.aspx?n_IdPedido=" + n_IdPedido + "&td=2";
            }
        }
    }
