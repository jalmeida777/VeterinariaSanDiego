...
public partial class ListarEmpresas : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Page.IsPostBack == false)
        {
            ...
            if (lblTitulo != null)
            {
                lblTitulo.Text = "Administración de Empresas";
            }
            Listar();
        }
        txtBuscar.Focus();
    ...
    void Listar()
    {
        ...
        SqlDataAdapter da = new SqlDataAdapter("PLAY_Empresa_Listar '" + txtBuscar.Text.Trim() + "'", conexion);
        da.Fill(dt);
    ...
    protected void gvAlmacen_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            int i_IdEmpresa = int.Parse(gvAlmacen.DataKeys[e.Row.RowIndex].Value.ToString());
            ImageButton btnEditar = e.Row.FindControl("ibEditar") as ImageButton;

            if (btnEditar != null)
            {
                btnEditar.PostBackUrl = "Empresa.aspx?i_IdEmpresa=" + i_IdEmpresa;
            }
